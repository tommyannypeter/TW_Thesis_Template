...
\def\degree    {碩士} % 碩士/博士
\def\studentID {105022519} % XD
\def\titleZh   {尋找與衰變成一對底夸克對的希格斯粒子相伴產生的$Z^\prime$-2HDM模型的暗物質}   % Chinese title
\def\titleEn   {Search for Dark Matter of $Z^\prime$-2HDM Model Produced in Association with MonoH Higgs Decaying to $b\bar{b}$} % English title
...
\def\profsZh   {徐百嫻, 你的共同指導, 可再加更多人\dots, 英文逗號示範} 
\def\profsEn   {Pai-Hsien Jennifer Hsu, Committee member, More member\dots , ZUO{,} GONG-DE}
...
\def\keywordsZh{暗物質, 希格斯玻色子, 大型強子對撞機, ATLAS}
\def\keywordsEn{Dark Matter, Higgs Boson, LHC, ATLAS}
